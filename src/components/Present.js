...
const PresentItem = ({ className, present, onSelected, currentUserId }) => (
  <div className={className}>
    ...
    <div style={{ flex: "1" }}>
      <Title target="_blank" href={present.link}>{present.name}</Title>
      <Description>{present.description}</Description>
      <Button
        type={
          isMyPresent(present.selected, currentUserId) ||
          notChosen(present.selected)
            ? "active"
            : "disabled"
        }
        onClick={onSelected}
...
const Button = styled.button`
  ...
  text-shadow: 1px 1px 3px
    ${props => darken(0.5, buttonStyles[props.type].color)};
  ${props =>
    props.type === "active"
      ? `&:hover {
